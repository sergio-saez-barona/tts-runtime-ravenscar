...
package body TT_Utilities is
   ...
   begin
      case Kind is
         ...
         when Initial | Initial_Continuation | Initial_Sync =>
            declare
               WS_Type : TTS.Work_Slot_Type :=
                 (case Kind is
                    when Initial_Continuation => TTS.Continuation,
                    when Initial_Sync => TTS.Sync,
                    when others => TTS.Regular);
            begin
               New_Slot := new TTS.Initial_Slot (WS_Type);
            end;

         when Regular | Terminal | Continuation | Final | Sync | Optional =>
            declare
               WS_Type : TTS.Work_Slot_Type :=
                 (case Kind is
                    when Continuation => TTS.Continuation,
                    when Sync => TTS.Sync,
                    when others => TTS.Regular);
            begin
   ...
   is
   begin
      if Slot.all in TTS.Initial_Slot'Class then
         declare
            Init_Slot : TTS.Any_Initial_Slot := TTS.Any_Initial_Slot (Slot);
         begin
            Init_Slot.Criticality_Level := Criticality;
         end;
      end if;
