-----------------------------------------------------------
--  @file:   tts_plan_mcs_dual.ads
--  @input:  plan_mcs_dual.tts
--  @brief:  Plan for testing MCS support for multi-frame tasks
--           It uses a Dual Criticality scheme (LO, HI)
...
package TTS_Plan_MCS_Dual is
   ...
   --  The TT plan
   Plan_MCS_Dual : aliased TTS.Time_Triggered_Plan :=
     (-- 0-50
      --  01 Seq. 1, IMs part
      TT_Slot (Initial,       50 * ms, 1,
               Work_Durations => (LO => 50 * ms, HI => 50 * ms)),
      -- 50-400
      --  02 Seq. 2, IMs part
      ...
      TT_Slot (Initial,       50 * ms, 4,
               Work_Durations => (LO => 40 * ms, HI => zero)),
      -- 1500-1570
      --  10 F part of Seq. 5
