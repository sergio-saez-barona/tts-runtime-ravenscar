...
package body TTS_Example_Ctx is

   package TT_Plan is new TTS_Plan_Ctx (Priority'Last - 1);
   use TT_Plan;
   ...
   Example_Debug : constant Boolean := True;

   procedure Debug_Put_Line (S : String) with Inline is
   begin
      if Example_Debug then
         Put_Line (S);
      end if;
   end Debug_Put_Line;
   ...
   W1       :
     Initial_Final_TT_Task (Work_Id => 1, Task_State => W1_State'Access);
   ...
   begin
      Debug_Put_Line ("W1: Initial slot released at " & Time_Str (Now));
      --   0 => Em_In;  1 => Ho_In;
      Update_Stats_Data
   ...
   begin
      Debug_Put_Line ("W1: Final slot released at " & Time_Str (Now));
   ...
   begin
      Debug_Put_Line ("W2: Initial slot released at " & Time_Str (Now));
      --   4 => Em_IS;  5 => Ho_IS;
      Update_Stats_Data
        (Release_Labels (Index), Release_Data (Index), Release_Delay);
   end Initial_Code;
   ...
   begin
      Debug_Put_Line ("W2: Final slot released at " & Time_Str (Now));
      --   6 => Em_Sy;  7 => Ho_Sy;
      Update_Stats_Data
        (Release_Labels (Index), Release_Data (Index), Release_Delay);
      S.Round := S.Round + 1;
   ...
   W3       : Simple_TT_Task (Work_Id => 3, Task_State => W3_State'Access);
   ...
   begin
      Debug_Put_Line ("W3: Main slot released at " & Time_Str (Now));
      --   8 => Em_IC;  9 => Ho_IC
      Update_Stats_Data
        (Release_Labels (Index), Release_Data (Index), Release_Delay);
      S.Round := S.Round + 1;
   ...
   W4       :
     Initial_Final_TT_Task (Work_Id => 4, Task_State => W4_State'Access);

   procedure Initial_Code (S : in out In_Co_Task_State) is
   begin
      Debug_Put_Line ("W4: Initial slot released at " & Time_Str (Clock));
   end Initial_Code;
   ...
   begin
      Debug_Put_Line ("W4: Final slot released at " & Time_Str (Now));
   ...
   W5 : Simple_TT_Task (Work_Id => 5, Task_State => W5_State'Access);
   ...
   begin
      ...
      Debug_Put_Line ("W5: Main code completed at " & Time_Str (Clock));
   end Main_Code;
   ...
   W6 : Simple_TT_Task (Work_Id => 6, Task_State => W6_State'Access);
   ...
   Init_Plan_Task  :
     Simple_TT_Task (Work_Id => 7, Task_State => Init_Plan_State'Access);

   procedure Main_Code (S : in out Init_Plan_Task_State) is
      Now : Time := Clock;
   begin
      Debug_Put_Line ("Init_Plan_Task: Plan starts at " & Time_Str (Now));
      Debug_Put_Line
        ("Plan delay: "
   ...
   -- begin
   --    Debug_Put_Line ("Background: Starting at " & Time_Str (Clock));
   --    loop
   --       delay until Clock + Milliseconds (50);
   --       Debug_Put_Line ("Background: Tick at " & Time_Str (Clock));
   --    end loop;
   -- end Background;
   ...
   begin
      TTS.Set_Plan (Plan_Ctx'Access, Start_Time);
   end Main;
